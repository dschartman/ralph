...
class TestGitBranchManager:
    """Tests for GitBranchManager class."""

    def test_worktree_path_includes_run_id(self):
        """Test that worktree paths include run_id to prevent conflicts."""
        from ralph2.git_manager import GitBranchManager

        run_id = "ralph2-abc123"
        work_item_id = "task-xyz789"
        project_root = "/project/root"

        manager = GitBranchManager(project_root=project_root, run_id=run_id)
        worktree_path = manager.get_worktree_path(work_item_id)

        # Path should include run_id
        assert run_id in worktree_path
        # Path should also include work_item_id
        assert work_item_id in worktree_path

    def test_branch_name_includes_run_id(self):
        """Test that branch names include run_id to prevent conflicts."""
        from ralph2.git_manager import GitBranchManager
        ...
        manager = GitBranchManager(project_root=project_root, run_id=run_id)
        branch_name = manager.get_branch_name(work_item_id)

        # Branch name should include run_id
        assert run_id in branch_name

    def test_cleanup_on_partial_failure(self):
        """Test that cleanup is guaranteed when worktree creation partially succeeds."""
        from ralph2.git_manager import GitBranchManager

        with patch("subprocess.run") as mock_run:
            # Setup: branch creation succeeds, worktree creation fails
            def side_effect(*args, **kwargs):
                cmd = args[0]
                if "branch" in cmd:
                    return MagicMock(returncode=0)
                elif "worktree" in cmd and "add" in cmd:
                    raise subprocess.CalledProcessError(1, cmd, stderr="Worktree failed")
                else:
                    return MagicMock(returncode=0)

            mock_run.side_effect = side_effect

            manager = GitBranchManager(project_root="/project/root", run_id="ralph2-test")

            # Should raise error but cleanup should happen
            with pytest.raises(RuntimeError):
                manager.create_worktree("task-123")

            # Verify cleanup was called (branch deletion)
            cleanup_calls = [call for call in mock_run.call_args_list
                           if "-D" in str(call) or "branch" in str(call)]
            # Should have attempted to clean up the branch
            assert len(cleanup_calls) >= 1

    def test_context_manager_cleanup(self):
        """Test that GitBranchManager can be used as context manager with cleanup."""
        from ralph2.git_manager import GitBranchManager
